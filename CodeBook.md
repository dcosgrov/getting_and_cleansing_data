...
Below is a an inventory of variables/column names within the dataset that is captured in the summary_data.out file generated by run_analysis.R.  Note that each variable has a comment “prefixed by #” that indicates its corresponding, underlying source variable name.  For the features listed below, keep in mind that the source variables referenced in the comments reflect the raw data variables within the X_train.txt and X_test.txt source files.  For the summary variable names below, the following conventions were used to map source variable names to the final summary variable names:
